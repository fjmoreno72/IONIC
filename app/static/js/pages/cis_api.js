...
const CISApi = {
    ...
    fetchCISPlanData: async function() {
        try {
            ...
            if (treeElement) {
                treeElement.innerHTML = '<div class="text-center p-3"><div class="spinner-border text-primary" role="status"></div><div class="mt-2">Loading CIS Plan data...</div></div>';
            }
    ...
    deleteItem: async function(type, id, parentIds) {
        try {
            ...
            if (type === 'missionNetworks') {
                ...
                const missionNetworkId = parsedParentIds.missionNetworkId || 
                    (Array.isArray(parsedParentIds) ? parsedParentIds[0] : parsedParentIds);
    ...
    addHwStack: async function(missionNetworkId, segmentId, domainId, name, cisParticipantID) {
        try {
            ...
            if (!missionNetworkId || !segmentId || !domainId) {
                return {
                    success: false,
                    error: 'All parent IDs (mission network, segment, domain) are required',
                    status: 400
                };
            }
